...
st.title('Vaccination Slots Availability Checker - CoWIN')
st.info('The CoWIN APIs are geo-fenced ! Try filling the details ')

valid_states = list(np.unique(mapping_df["state_name"].values))

left_column_1, center_column_1, right_column_1 = st.beta_columns(3)
with left_column_1:
    numdays = st.slider('Set Limit', 0, 100, 3)
...
if (final_df is not None) and (len(final_df)):
    ...
    st.error("Unable to fetch data, please try after a while")
